...
struct node{
         char ssn[25],name[25],dept[10],designation[25], phone[10];
         int sal;
         struct node *llink;
         struct node *rlink;
};
...
void display(){
        ...
        while(cur!=NULL){
        	cout<<"\nENode:" <<nodeno <<"||SSN:" <<cur->ssn <<"|Name:" <<cur->name <<"|Department:" <<cur->dept <<"|Designation:" <<cur->designation <<"|Salary:" <<cur->sal <<"|Phone no:" <<cur->phone;
            cur = cur->rlink;
            nodeno++;
        }
